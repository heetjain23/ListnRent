...
const ListingCard = ({ listing }) => {
  ...
  const locationDisplay = listing.location?.area
    ...
    : listing.location || "";
  const available =
    listing.isActive !== undefined ? listing.isActive : listing.available;
  const ownerName =
    listing.owner?.displayName || listing.owner?.name || "Owner";
  ...
  const optimizedImageMobile = imageUrl
    ...
    : null;
  const optimizedImageTablet = imageUrl
    ? getOptimizedImageUrl(imageUrl, { width: 400, height: 533 })
    : null;
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden aspect-3/4">
        ...
        {/* Category Badge */}
        <span
          className="absolute top-3 left-3 text-xs font-medium bg-white/90 backdrop-blur-sm
      ...
      <div className="p-4">
        <div className="flex items-center gap-2 mb-1">
          <h3
            className="text-sm font-semibold text-[#1A1A1A] leading-snug line-clamp-2 flex-1"
